...
#UserMission
SQLMISSIONUSER = 'SELECT process.Employee_Id, employee.Name, mission.Title,mission.Point,\
            process.status,mission.StartDate, mission.EndDate, mission.Description,process.Process_Id\
            From((process\
            Inner join employee on process.Employee_Id = employee.Employee_Id )\
            Inner join mission on process.Mission_Id = mission.Mission_Id)\
                 where employee.Employee_Id = %s'
SQLCANCLEMISSION = 'UPDATE process SET process.status = 0 WHERE process.process_Id = %s'
...
# UPDATE PASSOWRD
SQLUPDATEPSW ='UPDATE employee SET employee.Password = %s WHERE employee.Email = %s'
# LOCK ACCOUNT
